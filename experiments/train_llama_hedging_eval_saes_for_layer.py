    ...
    for layer in LAYERS:
        for seed in SEEDS:
            cfg = BaseSAERunnerConfig(
                ...
                device="cuda",
                run_name=f"llama-hl-width-{WIDTH}-layer-{layer}-seed-{seed}-{time.strftime('%Y-%m-%dT%H:%M:%S')}",
                lr_warm_up_steps=0,
                lr_decay_steps=0,
            ...
            script_configs.append(
                TrainHedgingEvalSaelensSaeOptions(
                    base_sae_cfg=cfg,
                    base_output_path=f"{base_output_path}/hedging-layers/llama-3.2-1b/layer-{layer}/l1-{L1}/width-{WIDTH}/seed-{seed}",
